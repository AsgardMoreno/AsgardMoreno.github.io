...
function filter(){
  ...
  let characters = {
    species: ['alien', 'human', 'humanoid', 'unknown1', 'poopybutthole', 'mytholog', 'animal', 'vampire', 'robot', 'cronenberg', 'disease', 'parasite'],
    ...
    // || "animal" || "vampire" || "robot" || "cronenberg" || "disease" || "parasite"){
        else if(characters.species.indexOf(select.value) != -1){
          if (select.value === "unknown1"){
    ...
    else if(characters.gender.indexOf(select.value) != -1){
        document.querySelector('.DivR').innerHTML =
        ` `
